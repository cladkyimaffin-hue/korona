...
severity: "info"
problem: |
  Необходимость структурированной документации по развертыванию, настройке и обслуживанию гиперконвергентного кластера Proxmox VE с Ceph, сетевой отказоустойчивостью (Bonding) и обеспечением кворума через QDevice.
solution: |
  Создан комплексный набор Markdown-файлов с YAML-метаданными, описывающих аппаратную спецификацию, агрегацию каналов (LACP), настройку Ceph и пост-установочную автоматизацию.
root_cause: |
  Сложность архитектуры гиперконвергентного кластера требует четкой фиксации решений (например, отказ от локального RAID для 7 ТБ дисков, использование LACP для 10G сетей и DAC-кабелей вместо оптики).
# === AI-СПЕЦИФИЧНЫЕ ПОЛЯ ===
ai_summary: |
  Папка содержит полную документацию по кластеру Proxmox VE на базе серверов Huawei 2288H V5. Включает спецификации железа, маппинг портов, настройку сетевого бондинга (LACP), инструкции по развертыванию Ceph (с разделением дисков 480 ГБ и 7 ТБ), обеспечение кворума через QDevice и пост-установочные скрипты.
key_takeaways:
  - "Для 7 ТБ дисков не используется локальный RAID1, каждый диск создается как отдельный OSD в Ceph."
  - "Сеть 10G для Ceph и VM должна быть агрегирована через bond0 (mode 4 / 802.3ad) для отказоустойчивости."
...
---
# Индекс папки: Proxmox (Кластер, Ceph и Сеть)

**Описание:** Документация, логи и пошаговые инструкции по развертыванию и обслуживанию кластера Proxmox VE на базе серверов Huawei 2288H V5. Охватывает аппаратную спецификацию, агрегацию сетевых каналов (LACP), настройку гиперконвергентного хранилища Ceph, обеспечение кворума через QDevice, пост-установочную автоматизацию и конфигурацию HA/SDN.
